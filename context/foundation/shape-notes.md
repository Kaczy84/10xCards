...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "workflow friction — the task exists but takes too long; quality is not the bottleneck"
    - topic: "primary persona"
      decision: "self-directed adult learner (professional development, language learning, certification prep)"
    - topic: "insight"
      decision: "LLMs can now generate high-quality flashcards from raw text in seconds; existing tools pre-date practical AI generation and haven't caught up"
    - topic: "auth model"
      decision: "login — email + password only (no OAuth for MVP); each learner has an account; cloud-stored flashcard sets"
    - topic: "role model"
      decision: "flat — all authenticated users are equal; no admin panel for MVP"
    - topic: "MVP scope"
      decision: "full scope — all 5 capabilities in v1 (AI generation + manual CRUD + browse/edit/delete + user accounts + spaced repetition integration); user acknowledged sustained-effort cost"
    - topic: "timeline"
      decision: "4–6 weeks of after-hours work; user accepted"
  frs_drafted: 12
...
Acknowledged on 2026-05-26: 4–6-week MVP with two external integrations (LLM + spaced repetition algorithm) requires sustained after-hours dedication; user accepted.
